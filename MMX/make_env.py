...
def make_mmx_env(
    ...
    if type == "train":
        ...
        envs = SubprocVecEnv(
            [make_env_wrapped_func(state, checkpointed, i, scen, image) for i in range(num_envs)]
        )
        return envs

    # testing environment
    elif type == "test":
        env = make_recording_env(state=state, scen=scen, image=image)
        return env
...
def make_recording_env(state, scen, image):
    ...
    env = MegamanXObservationWrapper(env, frameskip_prob=0, image=image) # dont frameskip when testing agent
    env.num_envs = 1    # set this here so stable-retro doesnt warn about wrappers
    return env

def make_env_wrapped_func(state, checkpointed, i, scen, image):
    ...
    def func():
        ...
        env = MegamanXActionSpaceWrapper(env, simple=intro)
        env = MegamanXObservationWrapper(env, image=image)
